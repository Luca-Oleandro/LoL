...
def safe_request(url, headers):
    ...
    while True:
        ...
        try:
            ...
            logger.warning(f"network error ({e.__class__.__name__}), retrying in 10s")
            time.sleep(10)
            continue

        if response.status_code == 200:
            try:
                response.json()  # validate the body is actually parseable JSON
            except ValueError:
                # Treat rare but real empty/malformed body like a 5xx: log and retry.
                logger.warning("got 200 but empty/invalid JSON body, retrying in 10s")
                time.sleep(10)
                continue
            return response
        elif response.status_code == 429:
            retry_after = int(response.headers.get("Retry-After", 5))
            logger.warning(f"rate limit exceeded, waiting {retry_after}s (server-specified)")
            time.sleep(retry_after)
        elif 500 <= response.status_code < 600:
            logger.warning(f"server error {response.status_code}, retrying in 10s")
            time.sleep(10)
        else:
...
def fetch_and_archive_matches(matches, headers, conn):
    ...
    for match in matches:
        try:
            ...
            logger.error(f"Skipping match {match}: failed to process ({e})")
